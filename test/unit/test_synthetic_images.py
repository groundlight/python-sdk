...
without a live server. The endpoint's backend is not generally available yet, so this is the
...
from groundlight.synthetic_images import LENS_CONFIG_MAX_ENCODED_BYTES, SyntheticImageResult
...
def _capturing_transport(captured: dict, data: bytes = _RESPONSE_JSON, status: int = 200):
    """Return a fake RESTClientObject.request that records its args.

    Unlike the multipart endpoints, this one sends the image as the whole body, and the query
    string is assembled inside the method being patched -- so capture `body` and
    `query_params` rather than `post_params` or the URL query.
    """
...
def test_returns_decoded_png_bytes(gl: ExperimentalApi):
    ...
    # Exact equality proves we b64-decoded rather than letting pydantic coerce the
    # base64 string into ASCII bytes.
...
def test_lens_config_dict_is_encoded_as_base64_json(gl: ExperimentalApi):
    ...
    assert encoded == base64.urlsafe_b64encode(b'{"a":1,"b":2}').decode("ascii")
...
def test_lens_config_accepts_json_string(gl: ExperimentalApi):
    ...
    assert dict(captured["query_params"])["lens_config"] == base64.urlsafe_b64encode(b'{"a":1,"b":2}').decode("ascii")


def test_lens_config_passes_through_preencoded_string(gl: ExperimentalApi):
    """An already-encoded value is forwarded unchanged."""
    pre_encoded = base64.urlsafe_b64encode(b'{"a":1}').decode("ascii")
    captured: dict = {}
    with patch.object(RESTClientObject, "request", _capturing_transport(captured)):
        gl.generate_synthetic_image(_FAKE_JPEG, lens_type="fence_climbing", lens_config=pre_encoded)

    assert dict(captured["query_params"])["lens_config"] == pre_encoded


@pytest.mark.parametrize("bad_config", ["{not valid json", "!!!not base64!!!", "cGxhaW4gdGV4dA=="])
def test_invalid_lens_config_raises_without_request(gl: ExperimentalApi, bad_config: str):
    """Bad configs fail locally rather than after a round trip."""
    captured: dict = {}
    with patch.object(RESTClientObject, "request", _capturing_transport(captured)):
...
def test_oversized_lens_config_raises_without_request(gl: ExperimentalApi):
    """The documented 1KiB limit is enforced client-side."""
    captured: dict = {}
    with patch.object(RESTClientObject, "request", _capturing_transport(captured)):
        with pytest.raises(ValueError, match="too large"):
            gl.generate_synthetic_image(
                ...
                lens_config={"padding": "x" * LENS_CONFIG_MAX_ENCODED_BYTES},
            )
