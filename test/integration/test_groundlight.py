...
def test_add_label_names(gl: Groundlight, image_query_yes: ImageQuery, image_query_no: ImageQuery, detector: Detector):
    ...
    iq_yes, iq_no = gl.submit_image_queries(detector=detector.id, image=transformed_yes_image), gl.submit_image_queries(
        detector=detector.id, image=transformed_no_image
    )
