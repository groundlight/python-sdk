...
#   timestamp: 2026-09-10T22:51:11+00:00
...
class ApiToken(BaseModel):
    ...
    last_used_at: Optional[datetime] = Field(
        ..., description="The most recent time this API token was used for authentication. Null until first use."
    )
    ...
    token_ttl: Optional[int] = Field(
        None,
        description=(
            "Identity token lifetime policy in whole seconds. Null means tokens minted under this identity never"
            " expire. Omitted only by older servers that do not yet expose this field."
        ),
    )
...
class ApiTokenCreationInputRequest(BaseModel):
    """
...
class DetectorStatusEnum(str, Enum):
    """
    * `ON` - ON
    * `OFF` - OFF
    """
...
class GLLModelInfo(BaseModel):
    """
    Lightweight pointer used by GLL clients to detect when the server has a
    newer model binary than the one they have cached locally. No S3 calls,
    no pre-signed URLs - one DB read.
    """
...
class Me(BaseModel):
    ...
    is_superuser: bool = Field(..., description="Whether the authenticated user has elevated (superuser) permissions.")
...
class PatchedDetectorRequest(BaseModel):
    ...
    status: Optional[Union[DetectorStatusEnum, BlankEnum]] = None
...
class StaleFromStatusEnum(str, Enum):
    """
    * `queued` - queued
    * `running` - running
    """
...
class DetectorCreationInputRequest(BaseModel):
    ...
    priming_group_id: Optional[constr(min_length=1, max_length=44)] = Field(
        None,
        description=(
            "ID of an existing PrimingGroup to associate with this detector (optional). Must be a priming group your"
            " account owns or a global one; any other ID is reported as not found."
        ),
    )


class GLLEngineInfo(BaseModel):
    """
    ...
    stale_from_status: Optional[StaleFromStatusEnum] = None
...
class GLLEngineInfoRequest(BaseModel):
    """
    Durable TensorRT build status and generation-scoped artifacts.
    """

    build_key: constr(min_length=1)
    status: Status638Enum
    stale_from_status: Optional[StaleFromStatusEnum] = None
    generation: Optional[int] = None
    task_id: Optional[constr(min_length=1)] = None
    attempt_count: int
    engine_url: Optional[constr(min_length=1)] = None
    engine_s3_key: Optional[constr(min_length=1)] = None
    metadata: Optional[Any] = None
    metadata_s3_key: Optional[constr(min_length=1)] = None
    model_binary_id: constr(min_length=1)
    compute_capability: constr(min_length=1)
    precision: constr(min_length=1)
    batch_size: int
    trt_version: constr(min_length=1)
    workspace_bytes: int
    engine_contract_version: constr(min_length=1)
    metadata_format_version: constr(min_length=1)
    error_code: Optional[constr(min_length=1)] = None
    error_message: Optional[constr(min_length=1)] = None
    updated_at: Optional[datetime] = None
    expires_at: Optional[datetime] = None


class GLLPipelineInfo(BaseModel):
    """
    ...
    oodd_model_binary_id: Optional[str] = Field(...)
    oodd_model_url: Optional[str] = Field(...)
