...
#   timestamp: 2026-09-11T22:30:13+00:00
...
class SyntheticImage(BaseModel):
    """
    Response shape for POST /v1/synthetic-images.

    A passthrough of the synthetic image service's generation result with a correlation
    `id` added.
    """

    id: str
    image: str = Field(..., description="Base64-encoded generated PNG.")
    width: int
    height: int
    label: str = Field(..., description='Lens-level binary event label: "YES" or "NO".')
    rois: List[ROI]
    added_roi_index: int = Field(
        ...,
        description=(
            "Index into `rois` of the object this request inserted. Every other entry was already in the frame."
        ),
    )
    metadata: Any = Field(..., description="Generation details reported by the generation service.")
