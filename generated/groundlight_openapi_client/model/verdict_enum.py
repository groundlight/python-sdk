...
class VerdictEnum(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """VerdictEnum - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): * `YES` - YES * `NO` - NO * `UNSURE` - UNSURE., must be one of ["YES", "NO", "UNSURE", ]  # noqa: E501

        Keyword Args:
            value (str): * `YES` - YES * `NO` - NO * `UNSURE` - UNSURE., must be one of ["YES", "NO", "UNSURE", ]  # noqa: E501
